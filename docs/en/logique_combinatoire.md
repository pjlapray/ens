...
!!! info "Application"
	* Routage de données : Sélectionner quelle variable afficher sur un écran unique.
	* Conversion parallèle-série : Il transforme des mots de données parallèles en un flux de données série.
### Le Démultiplexeur (DEMUX)
C'est l'inverse du MUX : il dirige la donnée d'entrée vers l'une des multiples sorties possibles.

!!! info "Application"
	* Adressage et sélection de mémoire : Il permet de choisir un registre ou une position de mémoire spécifique à lire ou à écrire dans un système.
	* Affichage multiplexé : Il pilote alternativement plusieurs afficheurs en commutant l'alimentation ou les données de manière séquentielle.
